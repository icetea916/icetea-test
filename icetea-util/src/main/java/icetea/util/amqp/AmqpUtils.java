    ...
    public void handleMessageWithRetry(Message message, String dlxRoutingKey, MessageProcessor messageProcessor) {
        ...
        try {
            ...
            if (nextRetryTimes > MAX_RETRY_TIMES) {
                // 超出最大尝试次数，将其放置到死信队列
                amqpTemplate.convertAndSend(DEFAULT_DLX, dlxRoutingKey, message);
//                LOGGER.error("消息 ID: {} 已超过最大重试次数，放置到死信队列中 {} ...", messageId, DEAD_LETTER_QUEUE);
            } else {
                // 入队重试
                amqpTemplate.convertAndSend(messageProperties.getReceivedExchange(), messageProperties.getConsumerQueue(),
                        message, (Message msg) -> {
                            // 更新重试次数
                            msg.getMessageProperties().setHeader(RETRY_HEAD_KEY, nextRetryTimes);
                            return msg;
                        });
//                LOGGER.info("消息 ID: {} 处理失败，进行第 {} 次重试至延时队列 {} ...", messageId, nextRetryTimes,
//                        DELAY_EXCHANGE);
            }
